...
DATA_LAKE_URL = 'dfs.core.windows.net'      # Azure Data Lake Storage Gen2
REGISTRY_URL = 'azurecr.io'                # Azure Container Registry
FRONTDOOR_URL = 'azurefd.net'              # Azure Front Door
API_MGMT_URL = 'azure-api.net'             # Azure API Management
BATCH_URL = 'batch.core.windows.net'       # Azure Batch
SEARCH_URL = 'search.windows.net'          # Azure AI Search
SIGNALR_URL = 'service.signalr.net'        # Azure SignalR Service
EVENT_HUB_URL = 'servicebus.windows.net'   # Azure Event Hubs / Service Bus
DEVOPS_URL = 'dev.azure.com'               # Azure DevOps
...
# --- 2026 New Service Implementation Functions & Callbacks ---

def print_generic_response(hostname, service_name):
    """
    Generic callback template for newly added Azure services
    """
    data = {'platform': 'azure', 'msg': f'Registered Azure {service_name} DNS Name', 'target': hostname, 'access': 'public'}
    utils.fmt_output(data)

def check_expanded_services(names, url_suffix, service_label, nameserver, threads, nameserverfile=False, alphanumeric_only=False):
    """
    A unified wrapper function to handle new 2026 Azure endpoints efficiently
    """
    print(f"[+] Checking for Azure {service_label}")
    start_time = utils.start_timer()
    ...
    for name in names:
        if alphanumeric_only and re.search(regex, name):
            continue
        candidates.append(f'{name}.{url_suffix}')

    utils.fast_dns_lookup(candidates, nameserver, nameserverfile,
                          callback=lambda host: print_generic_response(host, service_label),
                          threads=threads)
    utils.stop_timer(start_time)
...
def run_all(names, args):
    ...
    # --- 2026 Expanded Services Queue ---
    # Data Lake Storage Gen2 often shares the same character limits as standard storage accounts
    check_expanded_services(names, DATA_LAKE_URL, "Data Lake Gen2 Accounts", args.nameserver, args.threads, args.nameserverfile, alphanumeric_only=True)
    check_expanded_services(names, REGISTRY_URL, "Container Registries", args.nameserver, args.threads, args.nameserverfile)
    check_expanded_services(names, FRONTDOOR_URL, "Front Doors", args.nameserver, args.threads, args.nameserverfile)
    check_expanded_services(names, API_MGMT_URL, "API Management Services", args.nameserver, args.threads, args.nameserverfile)
    check_expanded_services(names, BATCH_URL, "Batch Accounts", args.nameserver, args.threads, args.nameserverfile)
    check_expanded_services(names, SEARCH_URL, "AI Search Services", args.nameserver, args.threads, args.nameserverfile)
    check_expanded_services(names, SIGNALR_URL, "SignalR Services", args.nameserver, args.threads, args.nameserverfile)
    check_expanded_services(names, EVENT_HUB_URL, "Event Hubs / Service Bus", args.nameserver, args.threads, args.nameserverfile)
    check_expanded_services(names, DEVOPS_URL, "DevOps Organizations", args.nameserver, args.threads, args.nameserverfile)