...
"""
blackcloud by initstring (github.com/initstring)
Modified & Enhanced by jakelo.ai

Multi-cloud OSINT tool designed to enumerate storage and services in AWS,
Azure, and GCP. (Formerly known as cloud_enum)
...
BANNER = '''
####################################################
        BLACKCLOUD - MULTI-CLOUD ENUMERATION
        https://github.com/jakeloai/BlackSecurity
####################################################
...
def parse_arguments():
    ...
    """
    desc = "Multi-cloud enumeration utility. Part of BlackSecurity."
    parser = argparse.ArgumentParser(description=desc)

    # Grab the current directory of this module to locate package resources reliably
    ...
    parser.add_argument('-m', '--mutations', type=str, action='store',
                        default=os.path.join(current_dir, 'enum_tools', 'fuzz.txt'),
    ...
    if not os.access(args.brute, os.R_OK):
        print(f"[!] Cannot access brute-force file, exiting: {args.brute}")
        sys.exit()
